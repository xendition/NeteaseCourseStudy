package com.michael.study.base.thread.threadpool;
 ...
 * <pre>
 *   线程池的使用
 *      Executors.newFixedThreadPool 固定线程数量的线程池，无界队列(实际使用时最好指定队列大小)
 *      Executors.newCachedThreadPool() 无核心线程+无任务队列存储 如果当前有空闲线程直接使用(60秒的过期时间)，没有则创建一个线程
 *      Executors.newScheduledThreadPool() 定时执行任务,周期执行任务
 *      Executors.newSingleThreadExecutor() 定时执行任务,周期执行任务
 * </pre>
 *
 * @author Michael
 * @version 1.0
 * @date 2019/04/27 12:47
 */
public class ThreadPoolDemo {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            threadPoolExecutor.submit(() -> {
                try {
                    ...
                    System.out.println(Thread.currentThread().getId() + " 开始执行：" + n);
                    Thread.sleep(3000L);
                    if (n %3 == 0) {
                        Thread.interrupted();
                    }
                    System.err.println(Thread.currentThread().getId() + " 执行结束:" + n);
    ...
    /** 单线程无界队列顺序执行线程池 线程执行异常也会继续处理 */
    private void threadPoolExecutorTest0() {
        ...
        // 查看线程数量，查看队列等待数量
        // Thread.sleep(500L);
        // System.out.println("当前线程池线程数量为：" + executorService.());
        // System.out.println("当前线程池等待的数量为：" + executorService.getQueue().size());
        // // 等待15秒，查看线程数量和队列数量（理论上，会被超出核心线程数量的线程自动销毁）
        // Thread.sleep(15000L);
        // System.out.println("当前线程池线程数量为：" + executorService.getPoolSize());
        // System.out.println("当前线程池等待的数量为：" + executorService.getQueue().size());
    }
    ...
    private void threadPoolExecutorTest3() throws Exception {
        // 和 Executors.newFixedThreadPool(int nThreads)一样的  固定线程量的线程池
        ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(
    ...
    private void threadPoolExecutorTest5() throws Exception {
        ...
        threadPoolExecutor.schedule(() -> System.out.println("任务被执行，现在时间：" + System.currentTimeMillis()), 3000,
    ...
    private void threadPoolExecutorTest7() throws Exception {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            System.out.println("任务提交成功 : 第" + i
                               + "个任务 核心线程数：" + threadPoolExecutor.getCorePoolSize()
                               + " 总线程数：" + threadPoolExecutor.getPoolSize()
                               + " 当前任务总数：" + threadPoolExecutor.getTaskCount()
                               + " 任务队列中的任务数：" + threadPoolExecutor.getQueue().size()
            );
        }
        // 1秒后终止线程池
        Thread.sleep(1000L);
